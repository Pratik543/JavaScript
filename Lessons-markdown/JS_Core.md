...
-   Lexical structure refers to the way in which instructions are written in the programming code.
-   It is pivotal to follow a precise and orderly structure so that the instructions are correctly understood by the machine.
-   This includes elements such as tokens, whitespace, comments, case sensitivity, and other features.

## - Tokens

```
Tokens are the building blocks of programming langugaes such as    R Reserved keywords    I Identifiers    O Operators(e.g. arithmetic, logical, comparison, assignment)    L Literals(text)    P Punctuation symbols
```

## Reserved Keywords

```
They cannot be used as identifiers for variables or functions. After all, they are reserved.eg. const, var, let    if, else, switch, case, default    for, while, do, break    function, return
```
...
-   The escape sequence `n` is used to insert a new line into a string:

```js
let myString = "Hellonworld!";//Here, the string `myString` would output as:Helloworld!
```
...
It is important to note that `const` does not mean that the value of the variable is immutable. It means that the variable identifier cannot be reassigned after its initialization, but the internal value can be changed, such as in the case of an object or arrays being modified.
...
1.  ***Primitive data types*** : These are the most basic types of data, and include `number`, `string`, `boolean`, `undefined`, `null`, `BigInt` and `symbol`.
2.  ***Non-Primitive(Object) data type***: This data type refers to more complex data structures that can be used to store collections of related data and behavior. Objects in JavaScript can be created using object literals, constructor functions, or classes.Ex: Arrays, Functions, Object.
3.  ***Special data types*** : These include `NaN` (Not a Number) and `Infinity`. But they lies under Primitive Type
...
A variable that has intentionally been assigned a value of null.
...
Not a Number - represents a value that is not a legal number.
...
A reusable block of code that can also be treated as a value.
...
A collection of key-value pairs with iterable access to its elements.
...
> 
> undefined because "javascript can not find value for this"
> 
> due to variable without value
> 
> due to missing function arguments
> 
> due to missing object properties
> 
> null "developer sets the value to 0"

> Note that JavaScript is a dynamically typed language, which means that the same variable can hold different data types at different times.

## Objects

```
Object is a data type that represents a collection of related data and behaviors. An object can contain properties that represent the state of the object, as well as methods that represent behaviors that the object can perform.- One of the main benefits of using objects in JavaScript is that they allow for encapsulation of data and functionality.- This means that data and behavior can be grouped together into a single entity, making it easier to manage and modify.- Additionally, objects can be used to model real-world entities, making it easier to understand and interact with complex systems.
```
...
```js
let person = {  name: 'John',  age: 30,  city: 'New York',  hobbies: ['reading', 'traveling', 'swimming'],};console.log(person.name); // Output: "John"console.log(person.hobbies[0]); // Output: "reading"person.age = 31;console.log(person.age); // Output: 31
```
...
```js
let person = {  name: 'John',  age: 30,  city: 'New York',  hobbies: ['reading', 'traveling', 'swimming'],  sayHello: function () {    console.log('Hello, my name is ' + this.name);  },};person.sayHello(); // Output: "Hello, my name is John"
```
...
```js
function Person(name, age, occupation) {  this.name = name;  this.age = age;  this.occupation = occupation;}let person = new Person('John', 30, 'Developer');
```

3.  Using the Object() constructor: Objects can be created using the Object() constructor. Here's an example:

```js
const person = new Object();person.name = 'John';person.age = 32;person.city = 'New York';
```
...
```js
class Person {  constructor(name, age, occupation) {    this.name = name;    this.age = age;    this.occupation = occupation;  }}let person = new Person('John', 30, 'Developer');
```
...
Performs basic mathematical operations such as addition, subtraction, multiplication, division, and modulus.
...
Assigns values to variables.Addition assignment

`let x = 5; x += 2; (x is now 7)`
...
Concatenates strings or finds the length of a string.
...
Combines multiple conditions and returns a boolean value.
...
Not equal to (without type conversion)
...
-   In the first row, the `&&` operator is ***used to check if both comparisons are true*** (`10` is greater than `5` AND `5` is less than `20`). Since both comparisons are true, the result is `true`.
-   In the second row, the `||` operator is ***used to check if at least one of the comparisons is true*** (`10` is greater than `5` OR `5` is greater than `20`). Since the first comparison is true, the result is `true`.
-   In the third row, the `!` operator is ***used to flip the boolean value of the comparison*** `10 > 5`. Since `10 > 5` is true, using the `!` operator gives us `false`.
...
```
 A block of code that performs a specific taskSteps01 Declaration02 Parameters03 Body which contains the code04 Function Call aka Invokation
```

#### Good Practices

1.  Naming conventions for functions should start as a verb makes the code more readable show , get , calc.  
...
```js
function name(params) {  //statement  //have access to "this" keyword}
```
...
```js
const newname = function (params) {  //statement};
```
...
```js
const Newname = (params) => {  //statement};
```

### Examples

```js
//Normal Function examplefunction sayHi(name) {console.log(`Hello ${name}`);// return}//example2function square(num) {// Declaration followed by function name(parameters)return num * num; //statements}const result = square(50); //function expressionconsole.log(result);
```

```js
//Function Expressionconst add = (a, b) => {  return a + b;};const sum = add(38, 42);console.log(sum);
```
...
```js
//Arrow Functionconst square1 = (number) => {  return number * number;};const result1 = square1(34);console.log(result1);// Shorthand of Arrow function if only one line of code is to be executed// sometimes if you only have one parameter you can remove the paratheses like const height = meter => console.log(meter) then you can call itconst square2 = (numb) => numb * numb;const result2 = square2(4);console.log(result2); //16
```
...
```javascript
function Car(make, model, year) {  this.make = make;  this.model = model;  this.year = year;  this.start = function () {    console.log('Starting the ' + this.make + ' ' + this.model);  };}var myCar = new Car('Toyota', 'Corolla', 2023);myCar.start(); // Output: Starting the Toyota Corolla
```
...
```javascript
class Car {  constructor(make, model, year) {    this.make = make;    this.model = model;    this.year = year;  }  start() {    console.log(`Starting the ${this.make} ${this.model}`);  }}let myCar = new Car('Toyota', 'Corolla', 2023);myCar.start(); // Output: Starting the Toyota Corolla
```
...
Instantiation involves allocating memory space for the object and initializing its state variables or attributes. Once an object or instance is instantiated, it can be manipulated and accessed throughout the program.  
***Overall, instantiation is an important concept in programming, as it is the process of creating new objects from class definitions and is used extensively in object-oriented programming.***
...
```js
   console.log(name); // Output: undefined   var name = "Alice";   console.log(name); // Output: AliceIn this example, the name variable is hoisted to the top of its scope. The first console.log statement outputs undefined because the variable is declared but not yet assigned a value. The second console.log statement outputs "Alice" after the value is assigned.
```
...
```js
   greet(); // Output: Hello!   function greet() {     console.log("Hello!");   }In this example, the greet function is hoisted to the top of its scope, so it can be called before its actual declaration in the code.//It's important to note that hoisting only moves the declarations to the top of the scope, not the initializations or assignments. It is considered a best practice to declare variables at the top of their scope to avoid any confusion or unexpected behavior caused by hoisting. Similarly, it's recommended to define functions before using them to improve code readability and maintainability.
```
...
```js
   const person = new BankAccount(); // ReferenceError: BankAccount is not defined   class BankAccount {     // ...   }In this example, trying to instantiate the BankAccount class before its declaration will result in a ReferenceError because the class declaration is not hoisted.
```
...
```js
   const person = new BankAccount(); // TypeError: Person is not a constructor   const BankAccount = class {     // ...   };In this example, trying to instantiate the BankAccount class expression before its declaration will result in a TypeError because the variable BankAccount is declared but not yet assigned a value.
```
...
```js
class BankAccout {  constructor(customerName, balance = 0) {    this.customerName = customerName;    this.accountNumber = Date.now();    this.balance = balance;  }  deposit(amount) {    this.balance += amount;  }  withdraw(amount) {    this.balance -= amount;  }}const pratik = new BankAccout('Pratik', 4_000);const shreya = new BankAccout('Shreya', 0);shreya.deposit(5000);console.log(pratik);console.log(shreya);
```
...
```js
class MathUtils {  static add(x, y) {    return x + y;  }}console.log(MathUtils.add(3, 5)); // Output: 8
```
...
```js
class Student extends Person {constructor(name, age, grade) {super(name, age);this.grade = grade;}     sayHello() {       super.sayHello();       console.log(`I am in grade ${this.grade}.`);     }}In this example, the Student class extends the Person class, inheriting its properties and methods. The super keyword is used to call the parent class constructor and method.
```
...
```js
const person = new Person("Alice", 25);person.sayHello(); // Output: Hello, my name is Alice and I am 25 years old.In this example, a new Person object person is created with the name "Alice" and age 25. The sayHello method can be called on the person object.
```
...
# 7. Reference and Value

```
When assigning [primitive](#1-variables-and-datatypes-in-js) data type value to a variable any changes are made directly to that value, without affecting original value.But, when assigning non-primitive data type value to a variable is done by reference so any changes will affect all the references.
```

```js
// To let person = { name: 'bob' };let person2 = { ...person };person2.name = 'susy';console.log('the name of the first person is ${person.name}'); //is bobconsole.log('the name of the second person is ${person2.name}'); //is susy
```

**Preserving the Original Object**

In the previous example, using the spread syntax (`...`) created a copy of the `person` object. This allowed you to modify `person2.name` without affecting `person.name`(original data). However, what if you don't want to create a copy and instead want to prevent changes to the original object entirely?

Here are two approaches:

1.  **Object.freeze()**:
    
    The `Object.freeze()` method prevents any further modifications to the properties of an object. Once an object is frozen, trying to change its properties will result in a `TypeError`. Here's how you can use it:
    
    ```javascript
    let person = { name: 'bob' };Object.freeze(person);// Attempting to modify a frozen object throws a TypeErrorperson.name = 'susy'; // throws TypeErrorconsole.log('the name of the first person is still ${person.name}'); // Output: the name of the first person is still bob
    ```
    
    In this case, even though `person` is referenced by `person2`, any attempt to modify `person.name` will be blocked because the original object is frozen.
    
2.  **Const Keyword:**
    
    Declaring the object using the `const` keyword prevents reassignment of the reference itself. While you can still modify the properties within the object, you cannot assign a completely new object to the `person` variable. Here's an example:
    
    ```javascript
    const person = { name: 'bob' };// Modifying properties within the object is allowedperson.name = 'susy'; console.log('the name of the first person is now ${person.name}'); // Output: the name of the first person is now susy// Reassigning the variable to a new object is not allowed// person = { name: 'alice' }; // throws TypeError
    ```
    

Choose the approach that best suits your situation. If you want to completely prevent any modifications to the object's properties, use `Object.freeze()`. If you only want to prevent reassigning the reference to a new object entirely, use `const`.